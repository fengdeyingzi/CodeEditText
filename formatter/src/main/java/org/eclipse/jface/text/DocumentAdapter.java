...
public class DocumentAdapter implements IDocumentListener {
  ...
  public FindReplaceDocumentAdapter find() {
    return new FindReplaceDocumentAdapter(fDocument);
  }
  ...
  public char getChar(int index) {
    try {
      return getDocumentForRead().getChar(index);
	...
	private void fireTextChanged() {
		...
		if (fTextChangeListeners != null && fTextChangeListeners.size() > 0) {
			Iterator<TextChangeListener> e= new ArrayList<TextChangeListener>(fTextChangeListeners).iterator();
			while (e.hasNext())
				e.next().textChanged();
		}
	}
